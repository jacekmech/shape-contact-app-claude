...
## Shape Workflow Expectations

This repository is also meant to demonstrate Shape in practice.

When working under Shape:
- keep implementation aligned with the selected feature, slice, and task scope
- prefer small, reviewable batches
- do not mix unrelated changes into the same batch
- ensure accepted implementation can be committed cleanly
- update workflow artifacts when required by the approved work, without turning the repo into document-heavy overhead
